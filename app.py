...
<nav>
  <button onclick="showTab('manage')">食譜管理</button>
  <button onclick="showTab('browse')">瀏覽食譜</button>
...
<script>
    ...
    let recipes = [];
    let convertedIngredients = []; // To store for apply

    function showTab(tab) {
        document.querySelectorAll('.tab-content').forEach(s => s.classList.remove('active'));
        document.getElementById(tab).classList.add('active');
        if (tab === 'browse') loadRecipes();
    ...
    function applyToEditForm() {
        if (convertedIngredients.length === 0) return alert('請先計算換算結果');
